...
const heroSlides = [
  {
    ...
    image: 'https://images.unsplash.com/photo-1554896541-dff010081afe?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxtb2Rlcm4lMjBsaWJyYXJ5fGVufDF8fHx8MTc2MjM5MDAwNnww&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral',
  ...
  },
  {
    ...
    image: 'https://images.unsplash.com/photo-1709924168698-620ea32c3488?crop=entropy&cs=tinysrgb&fit=max&fm=jpg&ixid=M3w3Nzg4Nzd8MHwxfHNlYXJjaHwxfHxsaWJyYXJ5JTIwYm9va3MlMjByZWFkaW5nfGVufDF8fHx8MTc2MjQ4ODA2NXww&ixlib=rb-4.1.0&q=80&w=1080&utm_source=figma&utm_medium=referral',
...
function BookGrid({ title, books, viewAllLink }: BookGridProps) {
  return (
    <div className="mb-8">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-base" style={{ background: 'linear-gradient(135deg, #FFB366 0%, #FF8C42 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontWeight: 700 }}>{title}</h2>
      </div>
        ...
        {books.map((book, index) => (
          ...
          >
            <div className="rounded overflow-hidden mb-2 shadow-md hover:shadow-xl transition-all hover:scale-105 relative bg-white">
              ...
              {book.promotion && (
                <div className="absolute top-0 left-0 z-10">
                  <div className="text-white px-2 py-1 text-[10px] rounded-br-lg flex items-center gap-1 shadow-lg" style={{ background: 'linear-gradient(135deg, #FFB366 0%, #FF8C42 100%)' }}>
            ...
            <div className="px-0.5">
              <h3 className="text-sm line-clamp-2 mb-1.5 leading-tight text-foreground" style={{ fontWeight: 500 }}>{book.title}</h3>
              <p className="text-xs mb-1 text-muted-foreground">ผู้แต่ง: {book.author}</p>
              <p className="text-xs" style={{ background: 'linear-gradient(135deg, #FFB366 0%, #FF8C42 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontWeight: 600 }}>หมวด: {book.category}</p>
            </div>
          </Link>
        ))}
      </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-3">
            ...
            <div className="lg:col-span-2">
              <div className="relative rounded-lg overflow-hidden h-[200px] md:h-[280px] shadow-xl">
                <div className="absolute inset-0" style={{ background: heroSlides[currentSlide].bgGradient }}>
                ...
                <div className="relative h-full flex items-center justify-center p-4 md:p-6 text-center">
                  <div className="text-white">
                    <h1 className="text-2xl md:text-4xl mb-2 md:mb-3">{heroSlides[currentSlide].title}</h1>
                    <p className="text-base md:text-xl opacity-90">{heroSlides[currentSlide].subtitle}</p>
                  </div>
                </div>
            ...
            {/* Side Banners - 1/3 width, 2 rows */}
            <div className="flex flex-col gap-5 md:gap-4">
              {/* Banner 1 - Search Books - Google Apps Script Link */}
              <a 
                ...
                className="block"
              >
                <div className="rounded-lg p-4 md:p-5 text-white h-[95px] md:h-[135px] flex flex-col justify-between cursor-pointer hover:shadow-2xl transition-all hover:scale-[1.02] shadow-lg border border-white/20" 
                  style={{ 
                ...
                >
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <h3 className="text-sm md:text-base" style={{ fontWeight: 700 }}>ค้นหาหนังสือ</h3>
                    </div>
                    <p className="text-[11px] md:text-xs opacity-95 leading-relaxed">
                      ค้นหาหนังสือ ผู้แต่ง ISBN<br />
                      ระบบ Dewey Decimal
                    </p>
                  </div>
                  <button className="px-4 py-2 rounded-md text-xs md:text-sm hover:opacity-95 transition-all w-full shadow-md bg-white/95 hover:bg-white backdrop-blur-sm" 
                    style={{ 
                      color: '#FF6B35', 
                      fontWeight: 700,
              ...
              {/* Banner 2 - Late Returns - Google Apps Script Link */}
              <a 
                ...
                >
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <h3 className="text-sm md:text-base" style={{ fontWeight: 700 }}>รายชื่อนักเรียนค้างคืนหนังสือ</h3>
                    </div>
                    <p className="text-[11px] md:text-xs opacity-95 leading-relaxed">
                      ตรวจสอบสถานะการยืม<br />
                      รายชื่อค้างคืน
                    </p>
                  </div>
                  <button className="px-4 py-2 rounded-md text-xs md:text-sm hover:opacity-95 transition-all w-full shadow-md" 
                    style={{ 
